...
"",
 ...
 TQFrame,					//FRAME5,
 TQFrame,					  //FRAME5TITLE
 TQLabel,					    //LABFRAME5TITLE
 TQFrame,					//FRAME5_A,
 TQFrame,					  //FRAME5ROW_A,
 TQLabel,					    //LABROW_A,
 TQFrame,					  //FRAME5ROW1_A,
 TQLabel,					    //LABNOMBRE_A,
 TQLineEdit,					    //EDITNOMBRE_A,
 TQLabel,					    //LABNIF_A,
 TQLineEdit,					    //EDITNIF_A,
 TQFrame,					  //FRAME5ROW2_A,
 TQLabel,					    //LABDIREC_A,
 TQLineEdit,					    //EDITDIREC_A,
 TQLabel,					    //LABCP_A,
 TQLineEdit,					    //EDITCP_A,
 TQFrame,					  //FRAME5ROW3_A,
 TQLabel,					    //LABMUNI_A,
...
static int generated_defineMask(PARAM *p)
{
  ...
  pvSetText(p,LABTITLE,pvtr("APARTADO 5\nDI DEFINITIVO"));
  ...
  pvSetEnabled(p,BUT5,0);
  
  pvQFrame(p,FRAME5,CENTRALFORM,Box,Plain,2,1);
  pvSetGeometry(p,FRAME5,350,50,1100,650);
  pvSetFont(p,FRAME5,"Verdana",12,1,0,0,0);
  pvSetEditable(p,FRAME5,0);

  pvQFrame(p,FRAME5TITLE,FRAME5,Box,Plain,1,1);
  pvSetGeometry(p,FRAME5TITLE,0,0,1100,100);
  pvSetPaletteBackgroundColor(p,FRAME5TITLE,220,220,220);
  pvSetFont(p,FRAME5TITLE,"Verdana",12,1,0,0,0);

  pvQLabel(p,LABFRAME5TITLE,FRAME5TITLE); 	
  pvSetGeometry(p,LABFRAME5TITLE,10,5,1090,80);
  pvSetText(p,LABFRAME5TITLE,pvtr("5. INFORMACIÓN RELATIVA AL ORIGEN DEL TRASLADO: Cuando los traslados se refieran a \nresiduos recogidos directa o indirectamente por la entidad local, la información\nrelativa al origen serán los datos de la entidad local."));
  pvSetFont(p,LABFRAME5TITLE,"Default",15,1,0,0,0);

  pvQFrame(p,FRAME5_A,FRAME5,Box,Plain,1,1);
  pvSetGeometry(p,FRAME5_A,0,100,1100,250);
  pvSetFont(p,FRAME5_A,"Verdana",12,1,0,0,0);

  pvQFrame(p,FRAME5ROW_A,FRAME5_A,Box,Plain,1,1);
  pvSetGeometry(p,FRAME5ROW_A,0,0,1100,50);
  pvSetFont(p,FRAME5ROW_A,"Verdana",12,1,0,0,0);

  pvQLabel(p,LABROW_A,FRAME5ROW_A);
  pvSetGeometry(p,LABROW_A,10,5,1090,40);
  pvSetText(p,LABROW_A,pvtr("a) Información del centro productor o poseedor de residuos o de la instalación origen de traslado:"));
  pvSetFont(p,LABROW_A,"Default",12,1,0,0,0);

  pvQFrame(p,FRAME5ROW1_A,FRAME5_A,Box,Plain,1,1);
  pvSetGeometry(p,FRAME5ROW1_A,0,50,1100,50);
  pvSetFont(p,FRAME5ROW1_A,"Verdana",12,1,0,0,0);

  pvQLabel(p,LABNOMBRE_A,FRAME5ROW1_A);
  ...
  pvQLineEdit(p,EDITNOMBRE_A,FRAME5ROW1_A);
  pvSetGeometry(p,EDITNOMBRE_A,140,-1,609,50);
  pvSetFont(p,EDITNOMBRE_A,"Default",12,0,0,0,0);
  pvSetEditable(p,EDITNOMBRE_A,0);

  pvQLabel(p,LABNIF_A,FRAME5ROW1_A);
  pvSetGeometry(p,LABNIF_A,760,5,90,40);
  pvSetText(p,LABNIF_A,pvtr("N.I.F.:"));
  pvSetFont(p,LABNIF_A,"Default",12,0,0,0,0);

  pvQLineEdit(p,EDITNIF_A,FRAME5ROW1_A);
  pvSetGeometry(p,EDITNIF_A,860,-1,239,50);
  pvSetFont(p,EDITNIF_A,"Default",12,0,0,0,0);
  pvSetEditable(p,EDITNIF_A,0);  


  pvQFrame(p,FRAME5ROW2_A,FRAME5_A,Box,Plain,1,1);
  pvSetGeometry(p,FRAME5ROW2_A,0,100,1100,50);
  pvSetFont(p,FRAME5ROW2_A,"Verdana",12,1,0,0,0);

  pvQLabel(p,LABDIREC_A,FRAME5ROW2_A);
  pvSetGeometry(p,LABDIREC_A,10,5,135,40);
  pvSetText(p,LABDIREC_A,pvtr("Dirección:"));
  pvSetFont(p,LABDIREC_A,"Default",12,0,0,0,0);

  pvQLineEdit(p,EDITDIREC_A,FRAME5ROW2_A);
  pvSetGeometry(p,EDITDIREC_A,140,-1,609,50);
  pvSetFont(p,EDITDIREC_A,"Default",12,0,0,0,0);
  pvSetEditable(p,EDITDIREC_A,0);

  pvQLabel(p,LABCP_A,FRAME5ROW2_A);
  pvSetGeometry(p,LABCP_A,760,5,90,40);
  pvSetText(p,LABCP_A,pvtr("C.P.:"));
  pvSetFont(p,LABCP_A,"Default",12,0,0,0,0);

  pvQLineEdit(p,EDITCP_A,FRAME5ROW2_A);
  pvSetGeometry(p,EDITCP_A,860,-1,239,50);
  pvSetFont(p,EDITCP_A,"Default",12,0,0,0,0);
  pvSetEditable(p,EDITCP_A,0);  

  pvQFrame(p,FRAME5ROW3_A,FRAME5_A,Box,Plain,1,1);
  pvSetGeometry(p,FRAME5ROW3_A,0,150,1100,50);
  pvSetFont(p,FRAME5ROW3_A,"Verdana",12,1,0,0,0);

  pvQLabel(p,LABMUNI_A,FRAME5ROW3_A);
  pvSetGeometry(p,LABMUNI_A,10,5,135,40);
  pvSetText(p,LABMUNI_A,pvtr("Municipio:"));
  pvSetFont(p,LABMUNI_A,"Default",12,0,0,0,0);

  pvQLineEdit(p,EDITMUNI_A,FRAME5ROW3_A);
  pvSetGeometry(p,EDITMUNI_A,140,-1,240,50);
  pvSetFont(p,EDITMUNI_A,"Default",12,0,0,0,0);
  pvSetEditable(p,EDITMUNI_A,0);

  pvQLabel(p,LABPRO_A,FRAME5ROW3_A);
  pvSetGeometry(p,LABPRO_A,390,5,150,40);
  pvSetText(p,LABPRO_A,pvtr("Provincia:"));
  pvSetFont(p,LABPRO_A,"Default",12,0,0,0,0);

  pvQLineEdit(p,EDITPRO_A,FRAME5ROW3_A);
  pvSetGeometry(p,EDITPRO_A,530,-1,220,50);
  pvSetFont(p,EDITPRO_A,"Default",12,0,0,0,0);
  pvSetEditable(p,EDITPRO_A,0);  

  pvQLabel(p,LABCA_A,FRAME5ROW3_A);
  pvSetGeometry(p,LABCA_A,760,3,90,50);
  pvSetText(p,LABCA_A,pvtr("Comunidad\nAutónoma:"));
  pvSetFont(p,LABCA_A,"Default",12,0,0,0,0);

  pvQLineEdit(p,EDITCA_A,FRAME5ROW3_A);
  pvSetGeometry(p,EDITCA_A,860,-1,239,50);
  pvSetFont(p,EDITCA_A,"Default",12,0,0,0,0);
  pvSetEditable(p,EDITCA_A,0);  

  pvQFrame(p,FRAME5ROW4_A,FRAME5_A,Box,Plain,1,1);
  pvSetGeometry(p,FRAME5ROW4_A,0,200,1100,50);
  pvSetFont(p,FRAME5ROW4_A,"Verdana",12,1,0,0,0);

  pvQLabel(p,LABNIMA_A,FRAME5ROW4_A);
  pvSetGeometry(p,LABNIMA_A,10,5,135,40);
  pvSetText(p,LABNIMA_A,pvtr("NIMA:"));
  pvSetFont(p,LABNIMA_A,"Default",12,0,0,0,0);

  pvQLineEdit(p,EDITNIMA_A,FRAME5ROW4_A);
  pvSetGeometry(p,EDITNIMA_A,140,-1,240,50);
  pvSetFont(p,EDITNIMA_A,"Default",12,0,0,0,0);
  pvSetEditable(p,EDITNIMA_A,0);
  
  pvQLabel(p,LABREG_A,FRAME5ROW4_A);
  pvSetGeometry(p,LABREG_A,390,5,150,40);
  pvSetText(p,LABREG_A,pvtr("Nº inscripción\nen el Registro:"));
  pvSetFont(p,LABREG_A,"Default",12,0,0,0,0);

  pvQLineEdit(p,EDITREG_A,FRAME5ROW4_A);
  pvSetGeometry(p,EDITREG_A,530,-1,220,50);
  pvSetFont(p,EDITREG_A,"Default",12,0,0,0,0);
  pvSetEditable(p,EDITREG_A,0);  

  pvQLabel(p,LABCNAE_A,FRAME5ROW4_A);
  pvSetGeometry(p,LABCNAE_A,760,2,90,48);
  pvSetText(p,LABCNAE_A,pvtr("CNAE:"));
  pvSetFont(p,LABCNAE_A,"Default",12,0,0,0,0);

  pvQLineEdit(p,EDITCNAE_A,FRAME5ROW4_A);
  pvSetGeometry(p,EDITCNAE_A,860,-1,239,50);
  pvSetFont(p,EDITCNAE_A,"Default",12,0,0,0,0);
  pvSetEditable(p,EDITCNAE_A,0); 

  pvQFrame(p,FRAME5_B,FRAME5,Box,Plain,1,1);
  pvSetGeometry(p,FRAME5_B,0,350,1100,300);
  pvSetFont(p,FRAME5_B,"Verdana",12,1,0,0,0);

  pvQFrame(p,FRAME5ROW_B,FRAME5_B,Box,Plain,1,1);
  pvSetGeometry(p,FRAME5ROW_B,0,0,1100,50);
  pvSetFont(p,FRAME5ROW_B,"Verdana",12,1,0,0,0);

  pvQLabel(p,LABROW_B,FRAME5ROW_B);
  pvSetGeometry(p,LABROW_B,10,5,1090,45);
   pvSetText(p,LABROW_B,pvtr("b) Información de la empresa autorizada para realizar operaciones de tratamiento de residuos, incluido el \nalmacenamiento, en caso de que el origen del traslado sea una instalación de tratamiento de residuos:"));
  pvSetFont(p,LABROW_B,"Default",12,1,0,0,0);

  pvQFrame(p,FRAME5ROW1_B,FRAME5_B,Box,Plain,1,1);
  pvSetGeometry(p,FRAME5ROW1_B,0,50,1100,50);
  pvSetFont(p,FRAME5ROW1_B,"Verdana",12,1,0,0,0);

  pvQLabel(p,LABNOMBRE_B,FRAME5ROW1_B);
  ...
  pvQComboBox(p,COMBOPRODUCTOR1,CENTRALFORM,0,AtBottom);
  pvSetGeometry(p,COMBOPRODUCTOR1,40,150,300,40);
  pvSetFont(p,COMBOPRODUCTOR1,"Default",12,1,0,0,0);
  pvHide(p,COMBOPRODUCTOR1);

  pvQComboBox(p,COMBOPRODUCTOR2,CENTRALFORM,0,AtBottom);
  pvSetGeometry(p,COMBOPRODUCTOR2,40,400,300,40);
  pvSetFont(p,COMBOPRODUCTOR2,"Default",12,1,0,0,0);
  pvHide(p,COMBOPRODUCTOR2);
